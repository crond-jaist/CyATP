...
$old_jquery(function($){
	$old_jquery.fn.crossword = function(entryData) {
			/*
			...
			*/
			...
			// puzz.data = puzzleData;
			// append clues markup after puzzle wrapper div
			// This should be moved into a configuration object

			this.after('<div class="col-lg-5" id="puzzle-clues" style=" overflow:auto ; word-break:break-word"><h2>Across</h2><ul id="across"></ul><h2>Down</h2><ul id="down"></ul></div>');
				...
				calcCoords: function() {
					...
					*/
					for (var i = 0, p = entryCount; i < p; ++i) {		
						...
						$('#' + puzz.data[i].orientation).append('<li tabindex="1" data-position="' + i + '" style="list-style-type: none">' + (i+1) +'.  '+ puzz.data[i].clue + '</li>');
					}				
				...
				*/
				checkAnswer: function(e) {
					...
					if(valToCheck === currVal){	
						...
						if(solvedNumber == puzz.data.length){
							window.setTimeout(function () {
								alert('Congratulations！');
							},3000);

							//alert('Congratulations！')
							// $('<div>').appendTo('body').addClass('alert alert-success').html('Congratulations！').show(5000).delay(5000).fadeOut();
						}
					...
					// z是统计已经决解的个数，如果全部解决，弹窗提醒
					// z++;
					// console.log(z);
					//
					// if(z == puzz.data.length){
					// 	alert('Congratulations！')
					// }
			...
			var util = {
				...
				// checkFinished: function (e) {
				// 	// 如果所有input标签的class都是done，则可以判断ok
				// 	console()
				//
				// }
